...
function deleteDuplicata(ticket) {
  return emit('dbDuplicataDelete', ticket );
}
...
console.log('SndSrv.addIngredient ingre : ',item.ingredients);
  ...
  if (validated) {
    ...
    if (
      ...
      item.ingredients = _ventilationIngredientsSteps(item, produitSteps, tvaCat, commandeMode);
    } else {
...
function completeStep(step, item, produitSteps, tvaCat, commandeMode) {
...
function updateMode(mode, commande, data_catalogue) {
  ...
  __items = __items.map(itm => {
    ...
    if (__stepsDuProduit) {  
      __ritm.ingredients = _ventilationIngredientsSteps(__ritm, __stepsDuProduit, tva, mode);
    }
...
function _ventilationIngredientsSteps(item, produitSteps, tvaCat, mode) {
  ...
  produitSteps.forEach((step) => {
    if (
      ...
      if (__ing.length > 0)
        __ingredients = [
          ...__ingredients,
          ..._setSupplements(step.regles[0], __ing, tvaCat, mode),
        ];
    } else {
      step.regles.forEach((regle) => {
        __ing = item.ingredients.filter((ing) => regle.type === ing.type);
        if (__ing.length > 0)
          __ingredients = [...__ingredients, ..._setSupplements(regle, __ing, tvaCat, mode)];
      });
    }
  });
...
function _setSupplements(rule, ingredients, tvaCat, mode) {
...
function saveCommande(commande, catalogueReducer) {
  ...
  commande.items.forEach((itm) => {
    ...
    if (itm.produitid!=='frais') {
      prd = produits.find((p) => p.id === itm.produitid);
    } else {
      prd = {tva_id: itm.tva.id};
    }
    items.push({
      ...itm,
      tva: {
  ...
  const __c = {
    ...commande,
    items,
    total: _getCommandeTotal(commande.items, commande.modificateurs, commande.devise.symbole),
  };
  ...
  let  __commandeVentilee = _getVentilationTva(__c, catalogueReducer.tva, commande.devise.symbole);
...
function _getVentilationTva(commande, cataloguetva, symbolemonnaie) {
  ...
  let __cmddiscount_num = 0;
  let __cmddiscount_pct = 0;
...
console.log('🔅 PREMIERE PASSE');
  ...
  __cmd.items.forEach(itm => {
    ...
    // si le modificateur est en % on l'applique sur le HT et on calcule le TTC
    if (__modcoef_pct > 0) {
    ...
    itm.ingredients.forEach(ing => {
      ...
      let __hi = ( ing.supplement / (1 + Number(ing.tva.valeur)) ) * 100;
    ...
    if (moditm) {
      moditm.montant = Math.round(__sttcitm - __modttcitm) / 100;
      const moditemIndex = __cmd.modificateurs.findIndex(m => m.modificateur_id===moditm.modificateur_id);
      __cmd.modificateurs[moditemIndex] = moditm;
  ...
  // CALCUL DE LA REMISE SUR LE PANIER
  ...
  __cmd.items.forEach( (itm, i) => {
    ...
    // on recalcule le HT NON ARRONDI à partir du TTC et du taux de TVA
    let __ht = (itm.pu / (1 + __tx)) * itm.quantite;
    let __tva = ( itm.pu - (itm.pu / (1 + __tx)) ) * itm.quantite;
    ...
    // on voit s'il existe un modificateur pour l'item
    let moditem = _getModificateurForItem(__cmd.modificateurs, itm.itemid);

    let __itmdiscount_pct = 0;
    let __itmdiscount_num = 0;
    if (moditem) {
      const ispc = String(moditem.valeur).includes("%");
      // recup valeur numérique
      let val = Math.abs(ispc 
        ? Number(String(moditem.valeur).slice(0, -1))
    ...
    // d'abord la remise produit
    // si la remise de l'item est un coef
    if (__itmdiscount_pct > 0) {
      __modhtitm = __ht - (__ht * __itmdiscount_pct);
      __modttcitm = __modhtitm * (1 + __tx);
    }
    ...
    // si la remise de la commande est un coef
    if (__cmddiscount_pct > 0) {
      __modhtitm = __modhtitm - (__modhtitm * __cmddiscount_pct);
      __modttcitm = __modhtitm * (1 + __tx);
    }
    ...
    itm.ingredients.forEach((ing, j) => {
      ...
      // si la remise de l'item est un coef
      if (__itmdiscount_pct > 0) {
        __modhting = __iht - (__iht * __itmdiscount_pct);
        __modttcing = __modhting * (1 + __itx);
      }
      // si la remise de l'item est numéraire
      if (__itmdiscount_num > 0) {
      ...
      // si la remise de la commande est un coef
      if (__cmddiscount_pct > 0) {
        __modhting = __modhting - (__modhting * __cmddiscount_pct);
        __modttcing = __modhting * (1 + __itx);
      }
      // si la remise de la commande est numéraire
      if (__cmddiscount_num > 0) {
  ...
  if (modcmd) {
    ...
    if (__cmddiscount_num > 0) {
      __cmd.modificateurs[modcmdIndex].montant = Math.round((__cmdsoustotal_ttc * __cmddiscount_num)) / 100;
    }
    if (__cmddiscount_pct > 0) {
...
function _getModificateurForCmd(modificateurs) {
  return modificateurs.find(mod => (mod.item===null && mod.operation<0 && mod.type!=='frais'));
}
...
function setCommandeFromOrder(data, catalogueReducer, parametres, numero) {
  const __monnaie = parametres.financier.monnaie || {iso:'EUR',nom:'euro',nom_pl:'euros',symbole:'€'};
  const dataparam = {...data, devise: __monnaie};
  const commande = getNewCommande(dataparam);
          ...
          mod.selected_items.forEach((ing) => {
            ...
            if (ingredient) {
              ...
              item.ingredients.push({
                ...
                type: ingredient.type,
                qte: ing.quantity,
                prix: Number(ing.price.base_unit_price.amount / 100),
                puht: Number(ingredient.supplementArray_c[MODES[commande.mode]].ht / 100),
    ...
    data.payment.promotions.promotions.forEach(promo => {
      ...
      commande.modificateurs.push(
        addModificateur({
          item: null,
          ingredient: null,
          valeur: `${(- Number(promo.promo_discount_value / 100))}${__monnaie.symbole}`
        })
      );
...
function setCommandeFromAPI(data, catalogueReducer, parametres, numero) {
  ...
  commande.numero = numero;
  if (data.modificateurs && Array.isArray(data.modificateurs)) {
    commande.modificateurs = data.modificateurs.filter(m => m.operation<0);
  }
  // on met tous les produits dans le même array
  let produits = [];
  // for (let [key, value] of Object.entries(catalogueReducer.catalogue)) {
  ...
  if (data.modificateurs && data.modificateurs.length>0) {
    const modfrais = data.modificateurs.filter(m => m.operation>0);

    modfrais.forEach(f => {
      
      const __fttc = Math.abs( Number(String(f.valeur).slice(0, -1)) );
      const __ftva = f.taux;
      const __tht = __fttc / (1 + __ftva);
      const __cattva = Object.values(catalogueReducer.tva).find(t => Number(t.valeur)===__ftva);
      
      console.log('FRAIS TVA', f.taux, __cattva);
      const frais = {
        produitid: 'frais',
        nom: f.nom,
        prix: __fttc,
        pu: __fttc,
        puht: Math.round(__tht * 100) / 100,
        tva: { ...__cattva, id:__cattva.tva_id },
        composition: [],
        ingredients: [],
        steps: [],
        stepslength: 0,
      ...
      commande.items.push(frais);
    });
  }

  commande.total = _getCommandeTotal(commande.items, commande.modificateurs, commande.devise.symbole);
...
function checkMarketing(commande, reglescatalogue) {
  ...
  const symbolemonnaie = commande.devise.symbole || '€';
      ...
      if (promo.selection==="one") {
        ...
        Object.entries(produits).forEach(([produitid, prd]) => {
          ...
          if (qte_promo>0) {
            ...
            // calcul de la valeur :
            const ispc = String(promo.valeur).includes("%");
            const val = Math.abs(ispc 
              ? Number(String(promo.valeur).slice(0, -1))
              : Number(String(promo.valeur).slice(0, -symbolemonnaie.length))
            );
            ...
            modifiers.push({
              ...
              valeur: promoval+commande.devise.symbole
            });
          }
...
const _getCommandeTotal = (items, modificateurs, symboledevise) => {
  ...
  if (undefined !== items) {
    items.forEach((itm) => {
      ...
      if (__modificateur && __modificateur.operation<0) {

        
        const ispc = String(__modificateur.valeur).includes("%");
        const val = Math.abs(ispc 
          ? Number(String(__modificateur.valeur).slice(0, -1))
          : Number(String(__modificateur.valeur).slice(0, -symboledevise.length))
        );

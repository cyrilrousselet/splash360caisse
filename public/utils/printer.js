 ...
 * @return {[Printer]} printer  [the escpos printer instance]
 */
Printer.prototype.drawLine = function (str="-") {

  str = str.substr(0,1);

  const _maxchar = this.width / this._fontsize[0];

  // this.newLine();
  for (var i = 0; i < _maxchar; i++) {
    this.buffer.write(Buffer.from(str));
  }
  this.newLine();
...
Printer.prototype.size = function (width, height) {
  ...
  this._fontsize = [width, height];
  this.buffer.write(_.TEXT_FORMAT.TXT_CUSTOM_SIZE(width, height));

  return this;
};

Printer.prototype.fontSize = function (size) {
  let fontsize = '';
  switch (size.toUpperCase()) {
    case '2HEIGHT':
      fontsize = _.TEXT_FORMAT.TXT_2HEIGHT;
      this._fontsize = [1,2];
      break;
    case '2WIDTH':
      fontsize = _.TEXT_FORMAT.TXT_2WIDTH;
      this._fontsize = [2,1];
      break;
    case '4SQUARE':
      fontsize = _.TEXT_FORMAT.TXT_4SQUARE;
      this._fontsize = [2,2];
      break;
    case 'NORMAL':
    default:
      fontsize = _.TEXT_FORMAT.TXT_NORMAL;
      this._fontsize = [1,1];
      break;
  }
  // log.info('Printer.fontSize('+size+')', fontsize);
  
  this.buffer.write(fontsize);
  return this;
}
...
Printer.prototype.frame = function (text, frameElement='#', align="CENTER") {
  frameElement = frameElement.substr(0,1);


  const _maxchar = this.width / this._fontsize[0];

  this.drawLine(frameElement);
  this.align('CT');
  ...
  this.text(
    ...
    (new Array(_maxchar + 1 - (6*this._fontsize[0])).join(' ')) + 
    (frameElement+frameElement+frameElement)
  );

const __croppedText = this._completeRaw(text,'center',{width: this.width-6});

//  const __croppedText = ' '+text+' ';
...
Printer.prototype._completeRaw = function(string, alignment='left', options={}) {


  let __width = options.hasOwnProperty('width') ? options.width : this.width;
  __width = Math.ceil(__width/this._fontsize[0]);

  const __sp = __width - Math.ceil(String(string).length / this._fontsize[0]);
...
Printer.Image = require('../../node_modules/escpos/image');
Printer.command = require('../../node_modules/escpos/commands');
Printer.Printer2 = require('../../node_modules/escpos/promisify');
